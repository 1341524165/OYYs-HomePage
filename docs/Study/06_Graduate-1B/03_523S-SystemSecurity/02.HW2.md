...
## 2. Why is it vulnerable? Is it protected using one of the countermeasures we learned about? Which one(s)? How do you know? (2 points)
...
## 3. How would you exploit this vulnerability? (2 points)

Excecute the following commands to int `gdb` and set a breakpoint at the `getPassword` function:
```shell
gdb -q hw2
...
So we can see that the ESP is pointing to **0x80485af**, which can be found in the main function above:
```shell
0x080485af <+77>:	add    esp,0x10
```
...
## 4. There is a secret function in the program. What is the name of the function?  Construct an input that causes the secret function to execute. Once you have caused the secret function to execute, state all that the function does (as seen in the terminal, not all that it does in assembly code).
