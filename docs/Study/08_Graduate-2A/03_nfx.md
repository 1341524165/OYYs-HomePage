...
5. char \*\*a
    ...
    char _a不就是一个字符串数组，a代表首地址。那么char _ (*a)就是和char *a[]一样的数据结构
...
1. forward_list:单向链表，只支持单向顺序访问

1. deque:底层数据结构为一个中央控制器和多个缓冲区，支持首尾（中间不能）快速增删，也支持随机访问。

1. stack:底层一般用23实现，封闭头部即可，不用vector的原因应该是容量大小有限制，扩容耗时

1. queue:底层一般用23实现，封闭头部即可，不用vector的原因应该是容量大小有限制，扩容耗时（stack和queue其实是适配器,而不叫容器，因为是对容器的再封装）

1. priority_queue:底层数据结构一般为vector为底层容器，堆heap为处理规则来管理底层容器实现

1. set:底层数据结构为红黑树，有序，不重复。

1. multiset:底层数据结构为红黑树，有序，可重复。

1. map:底层数据结构为红黑树，有序，不重复。

1. multimap:底层数据结构为红黑树，有序，可重复。

1. hash_set:底层数据结构为hash表，无序，不重复。

1. hash_multiset:底层数据结构为hash表，无序，可重复 。

1. hash_map :底层数据结构为hash表，无序，不重复。

1. hash_multimap:底层数据结构为hash表，无序，可重复。
...
1. 支持随机访问的容器：string,array,vector,deque
